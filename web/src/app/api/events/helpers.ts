...
export async function requestEventsApi(
  path: string,
  accessToken: string,
  init: RequestInit,
): Promise<unknown> {
  ...
  for (const baseUrl of baseUrls) {
    try {
      const response = await fetch(`${baseUrl}/events${path}`, {
        ...init,
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new EventsApiError(
          getErrorMessage(payload, 'Falha ao consultar eventos.'),
          response.status || 500,
          clearCookie,
        )
      }
      ...
      if (isLastAttempt) {
        throw new EventsApiError('Não foi possível conectar ao serviço de eventos.', 502)
      }
    }
  }

  throw new EventsApiError('Falha ao consultar eventos.', 500)
}
