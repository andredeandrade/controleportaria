...
export async function requestReportsApi(
  path: string,
  accessToken: string,
  init: RequestInit,
): Promise<unknown> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ReportsApiError(
          getErrorMessage(payload, 'Falha ao consultar relatórios.'),
          response.status || 500,
          clearCookie,
        )
      }
      ...
      if (isLastAttempt) {
        throw new ReportsApiError('Nao foi possivel conectar ao servico de relatorios.', 502)
      }
    }
  }
}
