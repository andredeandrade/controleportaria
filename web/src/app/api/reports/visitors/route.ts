...
export async function GET(request: Request) {
  ...
  try {
    ...
    const forwarded = ['page', 'pageSize', 'name', 'document', 'day', 'month', 'from', 'to']
    ...
    return NextResponse.json(
      { message: getThrowableMessage(error, 'Erro inesperado ao consultar visitantes.') },
      { status: 500 },
    )
  }
}
